...
function PostThread() {
  ...
  const handleEdit = async () => {
    ...
    const trimmed = editBody.trim().slice(0, 1000);
    if (!trimmed) return;
    await updateDoc(doc(db, 'feedback_posts', postId), { body: trimmed });
    setPost((p) => p ? { ...p, body: trimmed } : p);
    setEditing(false);
  };
        ...
        {editing ? (
          <div className="mb-5">
            ...
            <div className="flex gap-2 mt-2">
              <button onClick={handleEdit} className="ps-btn ps-btn-sm">Save</button>
              <button onClick={() => setEditing(false)} className="ps-btn ps-btn-sm ps-btn-ghost">Cancel</button>
            </div>
          </div>
        ) : (
          <>
            <p className="text-[#1f1f1f] text-[15px] leading-relaxed whitespace-pre-wrap mb-2">
              {post.body}
            </p>
            {post.screenshotUrl && (
              <Image
            ...
            {!post.locked && user?.uid === post.userId && (
              <button
                onClick={() => { setEditBody(post.body); setEditing(true); }}
                className="text-xs text-[#6b6b6b] hover:text-[#0070cc] transition-colors mb-3"
              >
                ✏️ Edit post
              </button>
            )}
