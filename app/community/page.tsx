...
function PostCard({
  ...
  liked: boolean;
  onBookmark: (post: FeedbackPost) => void;
  bookmarked: boolean;
}) {
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="flex items-center gap-1 text-xs text-[#6b6b6b]">
              ...
              <button
                ...
                className={`flex items-center gap-1 text-xs font-medium transition-colors ${
                  bookmarked ? 'text-[#f59e0b]' : 'text-[#6b6b6b] hover:text-[#f59e0b]'
                }`}
...
export default function CommunityPage() {
  ...
  // Load bookmarked post IDs for current user
  useEffect(() => {
    if (!user) { setBookmarkedIds(new Set()); return; }
    const q = query(
      collection(db, 'feedback_bookmarks'),
      where('userId', '==', user.uid)
    );
    const unsub = onSnapshot(q, (snap) => {
      const ids = new Set<string>();
  ...
  const handleBookmark = async (post: FeedbackPost) => {
    if (!user) return;
    const bookmarkId = `${post.id}_${user.uid}`;
    const bookmarkRef = doc(db, 'feedback_bookmarks', bookmarkId);
    if (bookmarkedIds.has(post.id)) {
      await deleteDoc(bookmarkRef);
    } else {
      await setDoc(bookmarkRef, { postId: post.id, userId: user.uid, createdAt: new Date() });
    }
  };
  ...
  const trending = activeFilter === 'all'
    ? [...posts].filter((p) => !p.pinned).sort((a, b) => b.likes - a.likes).slice(0, 3)
    : [];
    ...
    <div className="min-h-screen bg-[#f5f7fa]">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 flex flex-col gap-6">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          ...
          {CATEGORIES.filter((c) => !c.founderOnly).map((c) => (
            <FilterPill
              key={c.value}
            ...
            <div className="flex flex-col gap-4">
              {pinned.map((p) => (
                <PostCard key={p.id} post={p} onLike={handleLike} liked={likedIds.has(p.id)}
                  onBookmark={handleBookmark} bookmarked={bookmarkedIds.has(p.id)} />
              ))}
            </div>
          </section>
        )}
        ...
        {!loading && (
          <section>
            {(pinned.length > 0 || trending.length > 0) && (
              <h2 className="text-xs font-bold uppercase tracking-widest text-[#6b6b6b] mb-3 px-1">
                Latest Posts
              </h2>
            )}
